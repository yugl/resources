...
function addClass(obj,classStr){
	...
	for ( var i=0;i<arr.length;i++ )
	{
		...
		{
			obj.className?obj.className += ' '+arr[i]:obj.className += arr[i];
		}
	}
	return obj;
}

//删除className  removeClass
function removeClass(obj,classStr){   
	var arr =classStr.split(' ');         
    if(arr.length){ 
        for(var i=0;i<arr.length;i++){
            var reg1 = new RegExp('\\b'+arr[i]+'\\b');
            if(reg1.test(obj.className)){
                var className = obj.className.split(' ');
                for(var j=0;j<className.length;j++){
                    if ( reg1.test(className[j]) )
                    {
...
/*
*在指定元素之后添加DOM
*appendAfter(newNode,targetNode)
*
*在指定元素之前添加DOM
...
var th_Dom = function(id){
	...
	return {
		...
		parseColor : function (color, flag) {//用法:parseColor("#111111",true) ==1118481  alert(th_Dom().parseColor(1118481,false)) ==#111111
		  if (flag === true) {
